...
y_pred_mlp = Loaded_mlp.predict(X_test)
...
cv_scores_mlp = cross_val_score(Loaded_mlp, X_test, y_test, cv=5)
...
plt.title('Confusion Matrix - MLPClassifier (Dataset 3)')
plt.xlabel('Predicted Label')
plt.ylabel('True Label')
plt.savefig('mlp_dataset3_confusion_matrix.png')
plt.show()
...
# Evaluate the RandomForestClassifier model on the test set
...
cv_scores_rf = cross_val_score(Loaded_rf, X_test, y_test, cv=5)
...
sns.heatmap(conf_matrix_rf, annot=True, fmt='d', cmap='Blues_r', cbar=False,
            xticklabels=['No Babydoll', 'With Babydoll'], yticklabels=['No Babydoll', 'With Babydoll'])
plt.title('Confusion Matrix - RandomForestClassifier (Dataset 3)')
plt.xlabel('Predicted Label')
plt.ylabel('True Label')
plt.savefig('random_forest_dataset3_confusion_matrix.png')
plt.show()
...
plt.savefig('mlp_vs_rf_roc_curve.png')
plt.show()
...
plt.ylim([0.0, 1.05])
plt.xlim([0.0, 1.0])
plt.title('Precision-Recall Curve - MLP vs RF')
plt.legend(loc="lower left")
plt.savefig('mlp_vs_rf_precision_recall_curve.png')
plt.show()
...
label_counts_mlp = y_test.value_counts()
...
plt.xlabel('Label')
plt.ylabel('Count')
plt.title('Distribution of Actual and Predicted Labels for MLP and RF Classifiers')
plt.xticks(label_counts_mlp.index, ['No Babydoll', 'With Babydoll'])
plt.legend()

# Show plot
plt.tight_layout()
plt.savefig('label_distribution_bar_graph_combined.png')
plt.show()
