...
csv_files = list(filter(lambda f: f.startswith('withbaby_') and f.endswith('.csv'), all_files))
for i in csv_files:
...
fft_df['Infant_Presence'] = 1  # 1 for presence , and 0 for without baby case
...
np.save('withbaby_npy_array.npy', numpy_array)